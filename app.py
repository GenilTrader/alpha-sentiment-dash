...
st.markdown("""
    <style>
    .main-title { font-size:30px; font-weight:bold; color:#D4AF37; margin-bottom:5px; }
    .sub-title { font-size:15px; color:#94A3B8; margin-bottom:20px; }
    </style>
...
vetor_macro = st.sidebar.selectbox("Filtro de Pressão Sistêmica", ["Pressão Vendedora Ativa", "Pressão Compradora Ativa", "Regime de Neutralidade / Lateral"])
...
with col_text:
    ...
    analise_texto = st.text_area("Boletim Proprietário", height=350, placeholder="1. ARQUITETURA DE REGIMES DE PREÇO...\n2. ZONAS DE EXAUSTÃO DIÁRIA E VETORES DE ARBITRAGEM...")
...
if st.button("🔥 Compilar e Emitir Boletim Alfa [Acesso Restrito]", use_container_width=True):
    ...
    style_h2 = ParagraphStyle('H2', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=12, textColor=colors.HexColor('#D4AF37'), spaceBefore=15, spaceAfter=8)
    style_body = ParagraphStyle('Body', parent=styles['Normal'], fontName='Helvetica', fontSize=9, leading=14, textColor=colors.HexColor('#334155'), spaceAfter=6)
    ...
    elements.append(Paragraph(f"<b>Filtro de Pressão Sistêmica:</b> {vetor_macro}", style_body))
    ...
    # Cláusula de Direitos e Confidencialidade (Gera Valor e Autoridade)
    ...
    with open(pdf_filename, "rb") as file:
        st.download_button(
            label="📥 Baixar PDF Premium Pronto para Assinantes",
            data=file,
